...
def validate_gateway(gateway: dict) -> dict:
    ...
    # A simulated gateway has no hardware to point at (see simulation/). It still needs a
    # stable connection string - it identifies the gateway in its config entry and in the
    # device registry - so it gets the synthetic one of the simulation.
    if validated.get(CONF_SIMULATED):
        path = config_helpers.simulator_serial_path(validated[CONF_ID])
        validated[CONF_SERIAL_PATH] = path
        if GatewayDeviceType.is_lan_gateway(device_type):
            validated[CONF_GATEWAY_ADDRESS] = path
        return validated
...
def get_form_descriptor(hass: HomeAssistant) -> dict:
    ...
    from ..tools.gateway_scan import scan as scan_ports
    ...
    for gateway in (config.get(CONF_GATEWAY, []) or []):
        ...
        gateways.append({
            ...
            # a gateway without hardware: its devices are simulated (see simulation/). Every
            # list of the web ui marks it, so a simulated device is never mistaken for a real one.
            'simulated': bool(gateway.get(CONF_SIMULATED, False)),

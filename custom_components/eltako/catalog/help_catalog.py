...
from ..const import (CONF_EEP, CONF_SENDER, GatewayDeviceType, INTEGRATION_DIR, LOGGER,
                     PLATFORMS)
...
DOCS_INDEX_FILE = os.path.join(INTEGRATION_DIR, 'docs_index.json')
...
def _repository_root() -> str:
    ...
    return os.path.dirname(os.path.dirname(INTEGRATION_DIR))
...
def get_documentation(base_url: str = None) -> list[dict]:
    ...
    stale - regenerate it with `python -m custom_components.eltako.catalog.help_catalog`.
    """
